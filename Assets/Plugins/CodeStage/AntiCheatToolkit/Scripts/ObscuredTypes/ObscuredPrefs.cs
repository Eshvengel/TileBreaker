...
namespace CodeStage.AntiCheat.ObscuredTypes
{
	...
	public static class ObscuredPrefs
	{
		...
		private static string cryptoKey = ObscuredCryptoKeys.Prefs;
        ...
        private static Vector4 DecryptVector4Value(string key, string encryptedInput, Vector4 defaultValue)
        {
            if (encryptedInput.IndexOf(DEPRECATED_RAW_SEPARATOR) > -1)
            {
                ...
                float.TryParse(values[2], out z);
                float.TryParse(values[3], out w);
                var deprecatedResult = new Vector4(x, y, z, w);
                SetVector4(key, deprecatedResult);
                return deprecatedResult;
            }
	    ...
	    public static T GetJObject<T>(string key, bool compress = false)
	    {
	        if (!HasKey(key))
	            return default(T);
        ...
        public static void SetJObject<T>(string key, T obj, bool compress = false)
	    {
	        if (obj == null)
	        {
	            DeleteKey(key);
        ...
        #region IEnumerable
        public static T GetEnumerable<T, TK>(string key) where T : IEnumerable<TK>
	    {
	        return GetJObject<T>(key);
	    }

	    public static void SetEnumerable<T, TK>(string key, T enumerable) where T : IEnumerable<TK>
	    {
	        SetJObject(key, enumerable);
	    }

	    public static IEnumerable<T> GetEnumerable<T>(string key)
	    {
	        return GetEnumerable<IEnumerable<T>, T>(key);
        }

	    public static void SetEnumerable<T>(string key, IEnumerable<T> enumerable)
	    {
	        SetEnumerable<IEnumerable<T>, T>(key, enumerable);
	    }

        public static List<T> GetList<T>(string key, bool returnEmptyIfNull = false)
	    {
            var result = GetEnumerable<List<T>, T>(key);
	        if (result == null && returnEmptyIfNull)
	            result = new List<T>();
	        return result;
	    }
	    ...
	    {
	        SetEnumerable<List<T>, T>(key, list);
	    }

	    public static T[] GetArray<T>(string key, bool returnEmptyIfNull = false)
	    {
	        var result = GetEnumerable<T[], T>(key);
	        if (result == null && returnEmptyIfNull)
	            result = new T[0];
	        return result;
        }
	    ...
	    {
	        SetEnumerable<T[], T>(key, array);
	    }
        #endregion

	    #region Dictionary
        public static Dictionary<TKey, TValue> GetDictionary<TKey, TValue>(string key, bool returnEmptyIfNull = false)
	    {
	        var result = GetJObject<Dictionary<TKey, TValue>>(key);
	        if (result == null && returnEmptyIfNull)
	            result = new Dictionary<TKey,TValue>();
	        return result;
        }

	    public static void SetDictionary<TKey, TValue>(string key, Dictionary<TKey, TValue> dict)
	    {
	        SetJObject(key, dict);
	    }
	    #endregion
